...
namespace startechplus.ble
{
	...
	public class AndroidBleBridge : IBleBridge {
		...
		private bool needsDebug = false;

		private int scanPeriod = 60000;

		public void SetScanPeriod(int seconds){
			
			scanPeriod = seconds * 1000;

			if (bridge != null) {
				bridge.Call ("setScanPeriod", scanPeriod);
			} 
		}
		...
		public void EnableDebug(bool isDebug, Action<string> logAction){
			...
			if (bridge != null) {
				bridge.Call ("enableDebug", needsDebug);
			} 

			if (bluetoothDevice != null)
				bluetoothDevice.LogAction = logAction;
		}
