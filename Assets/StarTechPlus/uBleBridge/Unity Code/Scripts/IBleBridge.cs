...
namespace startechplus.ble
{
	...
	public interface IBleBridge 
	{
		...
		/**
		 * This function is used to set a timeout for scanning.  Set to 0 for infinate scan period
		 * @param seconds a Integer in seconds for the scan timeout period.
		 */
		void SetScanPeriod (int seconds);

		/**
		 * This function is used to enable native log output;
		 * @param isDebug a Boolean to enable or disable logging
		 * @param logAction an Action(string log) callback for log feedback from native code
		 */
		void EnableDebug(bool isDebug, Action<string> logAction);
