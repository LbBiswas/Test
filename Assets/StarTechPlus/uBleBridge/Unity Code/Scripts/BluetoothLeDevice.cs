...
namespace startechplus.ble
{
	...
	public class BluetoothLeDevice : MonoBehaviour
	{
		...
		public void OnDiscoveredDescriptor(string message)
		{
			...
			//Debug.Log("OnDiscoveredDescriptor " + tokens[0] +  ", " + tokens[1] +  ", " + tokens[2] +  ", " + tokens[3]));
			
			if(DiscoveredDescriptorAction != null)
			{
				if(isLowerCaseUUID)
					DiscoveredDescriptorAction(tokens[0].ToUpper(), tokens[1].ToUpper(), tokens[2].ToUpper(), tokens[3].ToUpper());
				else
					DiscoveredDescriptorAction(tokens[0], tokens[1], tokens[2], tokens[3]);
			}
